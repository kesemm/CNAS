...
dts)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 7 (Switch) Data - DTS Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
unzip)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 7 (Switch) Data - Unzip Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
wrongfile)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 7 (Switch) Data - Wrong File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
nozip)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 7 (Switch) Data - No Zip File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
Switch=$( cat ${WORKDIR}/${FileType}DAT/${FileType}.dat | grep -c 'REPORT OF SWITCHING ENTITY' )
...
cat ${WORKDIR}/${FileType}DAT/${FileType}.dat | sed 1,${LineNumber}d > ${WORKDIR}/${FileType}TEMP/Rows1
cat ${WORKDIR}/${FileType}TEMP/Rows1 | sed '$d' > ${WORKDIR}/${FileType}TEMP/Rows
...
cat ${WORKDIR}/${FileType}TEMP/Rows | cut -c476-486  |tr -s "[ ]" "[ ]" |sed -e "s/ $//g" >${WORKDIR}/${FileType}TEMP/HOST
...
# call DTSX
CMD /c "DTExec /FILE D:\SQLServer\DTSX\Import_LERG7_Data.dtsx"
...
blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 7 (Switch) Data Completed" -to browng,walshkel -server 192.168.10.51 -from database@leidos.ca
