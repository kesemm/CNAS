...
#	****************************************************************************************
#	* CVS File:      $RCSfile: ODDBallData.bash,v $
...
echo "LERG 6 (ODD Ball NXX) data file. The Error occured in the following module:" >> ${WORKDIR}/blatbody.txt
...
# 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (Odd Ball NXX) Data - DTS Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
# 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (Odd Ball NXX) Data - Unzip Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
# 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (Odd Ball NXX) Data - Wrong File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
# 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (Odd Ball NXX) Data - No Zip File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
${WORKDIR}/${FileType}TEMP/MAJOR_HORZ >${WORKDIR}/${FileType}TEMP/LERG6
...
RC_ST|\
MAJOR_VERT|\
MAJOR_HORZ" >${WORKDIR}/${FileType}TEMP/ODDBALL.txt

cat ${WORKDIR}/${FileType}TEMP/LERG6 >> ${WORKDIR}/${FileType}TEMP/ODDBALL.txt




# call DTSX
CMD /c "DTExec /FILE D:\SQLServer\DTSX\Import_ODDBall_Data.dtsx"
...
echo "  LERG 6 (ODD Ball NXX) Data Load Complete" >> ${WORKDIR}/blatbody.txt
echo >> ${WORKDIR}/blatbody.txt
echo >> ${WORKDIR}/blatbody.txt

blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (ODD Ball NXX) Data Completed" -to browng,walshkel -server 192.168.10.51 -from database@leidos.ca
