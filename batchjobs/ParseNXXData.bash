...
#	****************************************************************************************
#	* CVS File:      $RCSfile: ParseNXXData.bash,v $
...
echo "LERG 6 (NXX) data file. The Error occured in the following module:" >> ${WORKDIR}/blatbody.txt
...
dts)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (NXX) Data - DTS Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
unzip)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (NXX) Data - Unzip Module" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
wrongfile)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (NXX) Data - Wrong File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
nozip)
	...
	echo >> ${WORKDIR}/blatbody.txt
 	...
 	blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (NXX) Data - No Zip File" -to walshkel,browng -server 192.168.10.51 -from database@leidos.ca
...
RC_ST|\
MAJOR_VERT|\
MAJOR_HORZ" >${WORKDIR}/${FileType}TEMP/LERG6.txt

cat ${WORKDIR}/${FileType}TEMP/LERG6 >> ${WORKDIR}/${FileType}TEMP/LERG6.txt



# call DTSX
CMD /c "DTExec /FILE D:\SQLServer\DTSX\Import_LERG6_Data.dtsx"
...
echo "  LERG 6 (NXX) Data Load Complete" >> ${WORKDIR}/blatbody.txt
echo >> ${WORKDIR}/blatbody.txt
echo >> ${WORKDIR}/blatbody.txt

blat "${DOSWORKDIR}\blatbody.txt" -subject "Import LERG 6 (NXX) Data Completed" -to browng,walshkel -server 192.168.10.51 -from database@leidos.ca
