...
<title>Processing Timing For Update CO Codes Requests (in Calendar Days)</title>
<%
...
sqlQry = "SELECT [Month], [Year], IsNull(CAST([Min] as VarChar),'&nbsp;') as [Min], IsNull(CAST([Max] as VarChar),'&nbsp;') as [Max], IsNull(CAST(Round([Avg],2) as VarChar),'&nbsp;') as [Avg], IsNull(CAST(Round([STDEV],2) as VarChar),'&nbsp;') As [STDEV], [Sum] from CalendarDaysMonthlyUpdateCOCodes Where [Year]=2018 order by [Year] desc, [Order]"
 %>
...
<p align="center"><strong>Processing Timing For Update CO Codes Requests (in Calendar Days)</strong></p>
