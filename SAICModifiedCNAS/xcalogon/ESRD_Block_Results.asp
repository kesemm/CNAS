...
SQLNPAESRDQry = "SELECT Tix,NPA,xca_ESRD.NXX,ESRD,COStatusDescription,EntityName,xca_ESRD.OCN,PublicRemarks,ReserveID,CNARemarks FROM xca_ESRD Left Join xca_status_codes ON xca_ESRD.Status=xca_status_codes.COStatus Left JOIN xca_Entity ON xca_ESRD.EntityID=xca_Entity.EntityID WHERE (((xca_ESRD.NPA)='" & aNPA &"') AND ((xca_ESRD.ESRD) Like '" & aESRD & "%')) Order By xca_ESRD.NXX,ESRD;"
SET RSTNPAESRDQry = objConnection.execute(SQLNPAESRDQry)
%> </p>

<p align="center"><strong>CNAS NPA-ESRD Query </strong></p>
<b>
<% if RSTNPAESRDQry.EOF then %><b><p>No record found for:  <% = (aNPA) %> - <% = (aESRD) %> 
<%else%></p>

<p align="center"><strong>ESRDs Assigned to <% = (aNPA) %> - <% = (aESRD) %> Block</strong></p></td>
<b>
...
<% Do Until RSTNPAESRDQry.EOF %>
  ...
  <tr align="center">
	...
	<td>&nbsp;<%= RSTNPAESRDQry("NXX") %>&nbsp;</td>
    <td>&nbsp;<%= RSTNPAESRDQry("ESRD") %>&nbsp;</td>
	...
	<td>&nbsp;<%= RSTNPAESRDQry("CNARemarks") %>&nbsp;</td>
  </tr>
...
<p>Note: A ticket number of 999999999 implies a grandfathered ESRD.</p>
 <%end if%>
</p>
<%
objConnection.close %>
