...
<%
'****************************************************************************************
'* CVS File:      $RCSfile: CNAS_Remarks_Update.asp,v $
...
<p align="center"><strong>CNAS Remarks Update </strong></p>
...
SET objConnection = server.createobject("ADODB.connection")
objConnection.open "DSN=cnasadmin;UID=admin;PWD=cnasadmin"
aSQL = "UPDATE xca_COCode Set CNARemarks='"& aRemarks &"' WHERE (xca_COCode.NPA='" & aNPA & "' AND xca_COCode.NXX='" &  aNXX & "');" 
...
<p align="center">Sorry that code is not available to update. This normally means that it is in-service so we should not be playing with the effective date.</p>
<% end if%>
<% objConnection.close %>
</b>
<%
...
+"$RCSfile: CNAS_Remarks_Update.asp,v $\n"
+"$Revision: 1.2 $\n"
