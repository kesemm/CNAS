...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  Purpose:  This page gets the username and password from user and directs.  Will redirect them
    ...
    <%
      ...
      Select Case Cfg(cnnDB, "AuthType")
        ...
        Case 1	' NT Authentication
          ...
          Dim ntUserRes
          Set ntUserRes = SQLQuery(cnnDB, "SELECT sid FROM tblUsers WHERE username='" & username & "'")
          If ntUserRes.EOF Then
            Dim ntUpdRes, ntSidRes
            ntUpdRes = SQLQuery(cnnDB, "INSERT INTO tblUsers (sid, username) VALUES (" & GetUnique(cnnDB, "users") & ", '" & username & "')")
        ...
        Case 2	' DB Authentication
          If Request.Form("logon") = 1 Then
            Dim password
            username = Left(Lcase(Trim(Request.Form("username"))), 50)
            username = Replace(username, "'", "''")
            password = Left(Trim(Request.Form("password")), 50)
            
            Dim userRes
            Set userRes = SQLQuery(cnnDB, "SELECT sid,fname, password FROM tblUsers WHERE username='" & username & "'")
            If userRes.EOF Then
              invalid = TRUE
              url=""
            ...
            Else
              ...
              Session("logon_fname")=userRes("fname")
            End If
            userRes.Close
          Else
            Dim frm_url
        ...
        Case 3	' External Authentication
          ...
          Dim extUserRes
          Set extUserRes = SQLQuery(cnnDB, "SELECT sid FROM tblUsers WHERE username='" & username & "'")
'          If extUserRes.EOF Then
'            Dim extUpdRes, extSidRes
'            extUpdRes = SQLQuery(cnnDB, "INSERT INTO tblUsers (sid, username) VALUES (" & GetUnique(cnnDB, "users") & ", '" & username & "')")
'            extSidRes = SQLQuery(cnnDB, "SELECT sid FROM tblUsers WHERE username='" & username & "'")
