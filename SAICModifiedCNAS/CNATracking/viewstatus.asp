...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  Purpose:  This page lists the open problems based on status.
  -->
...
<head>
  <title><%=lang(cnnDB, "HelpDesk")%> - <%=lang(cnnDB, "ProblemStatus")%></title>
  <link rel="stylesheet" type="text/css" href="default.css">
</head>
<body>

<%
	Dim status_id,sname, rep_id, get_sname, sname_str

	if Len(Request.QueryString("status_id")) > 0 Then
		status_id = Cint(Request.QueryString("status_id"))
	Elseif Len(Request.Form("status_id")) > 0 Then
		status_id = Cint(Request.Form("status_id"))
	Else
		response.write "Error"
	End If
sname_str="Select sname From status where status_id=" & status_id
Set get_sname = SQLQuery(cnnDB, sname_str)
sname=get_sname("sname")
...
'		disp_total = FALSE
'	Else
		listStr = listStr & " p.status=" & status_id
		cntStr = cntStr & " status=" & status_id
		disp_total = TRUE
'	End If
...
<div align="center">
  ...
  <tr class="Head1">
    <td colspan="<%=strColumns%>">
      <%=lang(cnnDB, "ProblemStatus")%>&nbsp;<% = sname %>
      <%
        If disp_total Then
  ...
  <tr align="center" Class="Head2">
    <td nowrap><a href="viewstatus.asp?status_id=<% = status_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=1&order=<% = intIDOrder %>" class="HeadLink"><%=lang(cnnDB, "ID")%></a></td>
    <td><a href="viewstatus.asp?status_id=<% = status_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=2&order=<% = intTitleOrder %>" class="HeadLink"><%=lang(cnnDB, "Title")%></a></td>
    <td nowrap><%=lang(cnnDB, "AssignedTo")%></td>
    <td nowrap><a href="viewstatus.asp?status_id=<% = status_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=4&order=<% = intDateOrder %>" class="HeadLink"><%=lang(cnnDB, "DateSubmitted")%></a></td>
    <td><a href="viewstatus.asp?status_id=<% = status_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=5&order=<% = intDueDateOrder %>" class="HeadLink"><%=lang(cnnDB, "Duedate")%></a></td>
    <td><a href="viewstatus.asp?status_id=<% = status_id %>&start=<% = start %>&num=<% = numToDisplay %>&sort=7&order=<% = intPriOrder %>" class="HeadLink"><%=lang(cnnDB, "Priority")%></a></td>
  ...
  %>
    <div align="center">
    <% If start > 1 Then %>
      <A HREF="viewstatus.asp?status_id=<% = status_id %>&start=<% = startP %>&num=<% = numToDisplay %>&sort=<% = intSort %>&order=<% = intOrder %>">Previous</A>&nbsp;
    <% End If
      If Not (listRes.EOF) Then
    %>
      <A HREF="viewstatus.asp?status_id=<% = status_id %>&start=<% = startN %>&num=<% = numToDisplay %>&sort=<% = intSort %>&order=<% = intOrder %>">Next</A>
    <% End If %>
    </div>
  <%
  ...
  <tr class="Head1">
    <td colspan="6" valign="center">
      <font size="+2"><b><%=lang(cnnDB, "OpenProblemsfor")%>&nbsp;<% = sname %></b></font>
    </td>
  </tr>
  <tr align="center" Class="Head2">
